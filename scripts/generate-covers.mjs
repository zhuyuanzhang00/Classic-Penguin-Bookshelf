...
/** Cartouche cream locked to The Little Prince (id 17) so every cover shares one band system. */
const CLASSICS_PAPER = "#FEF3C7";
...
<svg xmlns="http://www.w3.org/2000/svg" width="400" height="620" viewBox="0 0 400 620" role="img" aria-labelledby="title">
  ...
  <rect x="18" y="18" width="364" height="584" fill="none" stroke="${paper}" stroke-width="2" opacity="0.55"/>
  <rect x="0" y="196" width="400" height="228" fill="${paper}"/>
  <rect x="0" y="196" width="400" height="8" fill="${book.ink}" opacity="0.85"/>
  <rect x="0" y="416" width="400" height="8" fill="${book.ink}" opacity="0.85"/>
  <text x="200" y="72" text-anchor="middle" fill="${paper}" font-family="Georgia, 'Times New Roman', serif" font-size="11" letter-spacing="5">CLASSICS</text>
  <text x="200" y="108" text-anchor="middle" fill="${paper}" font-family="Georgia, 'Times New Roman', serif" font-size="15" letter-spacing="3">${escapeXml(book.year)}</text>
  <ellipse cx="200" cy="148" rx="28" ry="34" fill="${paper}" opacity="0.95"/>
  <ellipse cx="200" cy="156" rx="16" ry="20" fill="${book.band}"/>
  <circle cx="193" cy="146" r="3" fill="${paper}"/>
  <circle cx="207" cy="146" r="3" fill="${paper}"/>
  <path d="M188 168 C200 176 212 176 212 168" fill="none" stroke="${paper}" stroke-width="2"/>
  ...
  <text x="200" y="520" text-anchor="middle" fill="${paper}" font-family="Georgia, 'Times New Roman', serif" font-size="12" letter-spacing="3">A SHELF EDITION</text>
  <text x="200" y="552" text-anchor="middle" fill="${paper}" font-family="Georgia, 'Times New Roman', serif" font-size="11" opacity="0.8">No. ${String(book.id).padStart(2, "0")}</text>
</svg>
`;
}
